...
// Cesty pro admin/index.php a admin/invoices.php
$config = require __DIR__ . '/../config.php';
require __DIR__ . '/../classes/ElectrumRPC.php';
require __DIR__ . '/../classes/ElectrumWallet.php';
require __DIR__ . '/../classes/BtcDashboard.php'; // v index.php
require __DIR__ . '/../classes/BtcInvoiceManager.php'; // v invoices.php
require __DIR__ . '/../classes/Database.php'; // v invoices.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'create') {
        ...
        $orderId = trim($_POST['order_id'] ?? ''); // Nyní přijímá i text (např. ORD-123)
        ...
        try {
            ...
            $res = $invoiceManager->createInvoice($amount, $desc, ['order_id' => $orderId], 15);
            
            $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? "https://" : "http://";
            $host = $_SERVER['HTTP_HOST'];
            $path = dirname($_SERVER['REQUEST_URI']);
            $newInvoiceUrl = $protocol . $host . rtrim($path, '/\\') . '/pay.php?inv=' . $res['token'];

            // OPRAVA 7.0E-8: Částka se ukládá natvrdo jako formátovaný text
            ...
            $toastMsg = "Faktura byla úspěšně vygenerována.";
        } catch (Exception $e) {
    ...
    elseif ($_POST['action'] === 'clear_history') {
        $_SESSION['created_invoices'] = [];
        $toastMsg = "Historie faktur byla vymazána.";
    }
}
?>
<!doctype html>
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    h1 { font-size: 26px; margin: 0; letter-spacing: -.5px; color: #17201a; }
    .ghost-btn { border: 1px solid #e5eae7; background: #fff; border-radius: 11px; padding: 10px 14px; color: #17201a; text-decoration: none; font-weight: 600; font-size: 13px; transition: 0.2s; display: inline-flex; align-items: center; gap: 6px; }
    ...
    label { display: block; font-size: 12px; font-weight: 600; margin-bottom: 7px; color: #748078; text-transform: uppercase; letter-spacing: 0.5px; }
    ...
    input { width: 100%; border: 0; outline: 0; padding: 13px; font: inherit; color: #17201a; background: transparent; }
    .unit { padding: 13px 15px; font-weight: 700; color: #748078; border-left: 1px solid #e5eae7; background: #f9fafa; }
    .primary { width: 100%; border: 0; background: #2fd35a; color: #fff; border-radius: 10px; padding: 13px; font-weight: 700; cursor: pointer; transition: 0.2s; font-size: 14px; }
    ...
    .invoice-item strong { font-size: 14px; }
...
<div class="container">
    ...
    <div class="card">
        <h2 class="card-title"><i class="fa-solid fa-file-invoice-dollar" style="color:#20b948;"></i> Vystavit novou fakturu</h2>
        <form method="POST">
            ...
            <div class="field">
                <label>Popis / Název položky pro zákazníka</label>
                <div class="input-wrap">
                    <input type="text" name="description" placeholder="Např. Hlídání dětí" required>
                </div>
            </div>
            <div class="field">
            ...
            <button type="submit" class="primary"><i class="fa-solid fa-plus"></i> Vytvořit platební odkaz (15 minut)</button>
        </form>
    ...
    <div class="card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            ...
            <form method="POST" style="margin:0;">
                ...
                <button type="submit" class="ghost-btn" style="padding: 6px 10px; font-size: 11px; border: none; color: #ef4d4d; background: #fff0f0;"><i class="fa-solid fa-trash"></i> Smazat</button>
            </form>
        </div>
        <div>
