...
<section class="card filter-card">
  <form method="get" action="<?php echo $invoicesUrl; ?>" class="filter-bar filter-bar-wide">
    <div class="field"><label for="invoiceClientFilter">Zákazník</label><div class="input-wrap"><select id="invoiceClientFilter" name="user_id">
      <option value="">Všichni zákazníci</option><option value="0" <?php echo $selectedUserId === 0 ? 'selected' : ''; ?>>Systém / bez klienta</option>
    ...
    <div class="field"><label for="invoiceStoreFilter">Obchod</label><div class="input-wrap"><select id="invoiceStoreFilter" name="store_id"><option value="">Všechny obchody</option><?php foreach ($filterStores as $store): ?><option value="<?php echo htmlspecialchars((string) $store['id'], ENT_QUOTES, 'UTF-8'); ?>" <?php echo $selectedStoreId === $store['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars((string) $store['name'], ENT_QUOTES, 'UTF-8'); ?></option><?php endforeach; ?></select></div></div>
    <div class="field"><label for="invoiceStatusFilter">Stav</label><div class="input-wrap"><select id="invoiceStatusFilter" name="status"><option value="">Všechny stavy</option><?php foreach ($invoiceStatuses as $status): ?><option value="<?php echo htmlspecialchars($status, ENT_QUOTES, 'UTF-8'); ?>" <?php echo $selectedStatus === $status ? 'selected' : ''; ?>><?php echo htmlspecialchars($status, ENT_QUOTES, 'UTF-8'); ?></option><?php endforeach; ?></select></div></div>
    <div class="filter-actions"><button type="submit" class="primary"><i class="fa-solid fa-filter"></i> Filtrovat</button><a class="ghost-btn" href="<?php echo $invoicesUrl; ?>">Zrušit</a></div>
  </form>
</section>

<section class="card">
  <div class="card-title"><span class="card-title-group"><i class="fa-solid fa-database"></i> Databázové faktury</span><span class="badge s-unknown"><?php echo count($databaseInvoices); ?></span></div>
  <p class="card-subtitle">Až 200 nejnovějších faktur podle zvoleného zákazníka, obchodu a stavu.</p>
  <?php if ($databaseInvoices === []): ?><div class="empty-state"><p>Filtru neodpovídají žádné faktury.</p></div><?php else: ?>
  <div class="data-table-wrap"><table class="data-table"><thead><tr><th>Čas</th><th>Zákazník</th><th>Obchod</th><th>Faktura</th><th>Částka</th><th>Stav</th><th>Platnost</th><th></th></tr></thead><tbody>
  <?php foreach ($databaseInvoices as $invoice): ?>
    <?php $statusClass = $statusClasses[(string) $invoice['status']] ?? 's-unknown'; ?>
    <tr>
      <td class="muted"><?php echo date('d.m.Y H:i:s', (int) $invoice['created_at']); ?></td>
      <td><?php echo htmlspecialchars((string) $invoice['client_email'], ENT_QUOTES, 'UTF-8'); ?></td>
      <td><strong><?php echo htmlspecialchars((string) $invoice['store_name'], ENT_QUOTES, 'UTF-8'); ?></strong></td>
      <td><code><?php echo htmlspecialchars((string) $invoice['id'], ENT_QUOTES, 'UTF-8'); ?></code></td>
